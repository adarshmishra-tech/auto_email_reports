...
class AutoEmailReports:
    ...
    def setup_gui(self):
        ...
        ttk.Label(frame, text="Sender Email:").grid(row=2, column=0, sticky=tk.W, pady=4)
        self.email = ttk.Entry(frame, width=40)
        self.email.insert(0, self.config['email'])
        self.email.grid(row=2, column=1, pady=4)
        ...
        self.attachments_label = ttk.Label(frame, text=", ".join(self.config.get('attachments', [])), wraplength=400)
    ...
    def validate_inputs(self):
        email_regex = r"[^@]+@[^@]+\.[^@]+"
        if not re.match(email_regex, self.email.get()):
            messagebox.showerror("Validation Error", "Sender email is invalid.")
            return False
